...
public class EffectData {
	...
	public String EDtoString(int id,int _duration,double _amplifier){
		if(id == 0){
			return "+" + _amplifier +  " 不明な上昇値_" +Converter.toTimeString(_duration/20);
		}else if(id == 1){
			return "+" + _amplifier +  " 接続人数から";
		}else if(id == 2){
			return "+" + _amplifier +  " 採掘量から";
		}else if(id == 3){
			return "+" + _amplifier +  " ﾄﾞﾗｹﾞﾅｲﾀｲﾑから_" + Converter.toTimeString(_duration/20);
		}else if(id == 4){
			return "+" + _amplifier +  " 投票ボーナスから_" + Converter.toTimeString(_duration/20);
		}else if(id == 5){
			return "+" + _amplifier +  " コマンド入力から_" + Converter.toTimeString(_duration/20);
		}
		return "+" + _amplifier +  " 不明な上昇値_" + Converter.toTimeString(_duration/20);
	}
}
