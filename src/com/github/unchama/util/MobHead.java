...
import com.github.unchama.yml.MobHeadDataManager;
import com.github.unchama.yml.MobHeadDataManager.MobHeadData;
...
//	};
	...
	static private MobHeadDataManager headDataManager = Gigantic.yml
			.getManager(MobHeadDataManager.class);

	private static HashMap<String, ItemStack> mobMap;

	public MobHead(){
		Map<String, MobHeadData> headData = headDataManager.getMap();
		for(String id : headData.keySet()){
			mobMap.put(id, getSkull(headData.get(id).url));
		}
	}
	...
	public static String getMobURL(String s) {
		return headDataManager.getData(s).url;
	}

	/**
	 * 与えられた文字に合致する頭の表示名を取得する．
	 *
	 * @param s
	 * @return
	 */
	public static String getMobName(String s) {
		return headDataManager.getData(s).name;
	}
