...
public final class GachaExchanger implements Exchanger {
	...
	public void exchange(Player player, ItemStack[] input) {
		...
		for (ItemStack item : input) {
			...
			if (!nbti.hasKey(Gacha.GACHATYPENBT) || !nbti.hasKey(Gacha.GACHARARITYNBT)
					|| !nbti.hasKey(Gacha.ROLLPLAYERUUIDNBT)) {
				outputItems.add(item);
				continue;
			}
			GachaType gtype = GachaManager.getGachaType(nbti);
			Rarity r = GachaManager.getGachaRarity(nbti);
			UUID uuid = GachaManager.getUUID(nbti);
			//対象アイテムはカウント
			if (canExchangeType.contains(gtype) && ticketMap.containsKey(r) && uuid != null
		...
		for (Rarity r : countMap.keySet()) {
			int inputAmount = countMap.get(r);
			int ticketAmount = ticketMap.get(r);
			player.sendMessage(ChatColor.GREEN + r.getRarityName() + "の景品 " + inputAmount + "個  × " + ticketAmount
					+ " = " + ticketAmount * inputAmount + "枚");
		}
